...
const day = async () => {
  ...
  const touchingSymbols = symbols.map((symbol) => {
    const touchingValues = values.filter((value) =>
      doesValueTouchSymbol(value, symbol)
    );

    if (touchingValues.length === 2) {
      return touchingValues[0].value * touchingValues[1].value;
    }

    return 0;
  });

  const sumOfTouchingValues = touchingSymbols.reduce((a, b) => a + b, 0);
